...
NMEADataSource::NMEADataSource(QObject *parent) : QObject(parent)
{
...
void NMEADataSource::start()
{
	...
	m_serialPort->setPortName("\\\\.\\CNCA0");
	...
	if (m_buffer.contains("\n"))
	{
		...
		if (line.startsWith("$GPGGA"))
		{
			...
			if (linesplit.size() > 9)
			{
				...
				emit location(lon,lat,time,sats,alt);
				/*DataClass data;
				data.latitude = lat;
				data.longitutde = lon;
		...
		else if (line.startsWith("$GPGSA"))
		{
			...
			if (linesplit.size() > 7)
			{
				int fixtype = linesplit[2].toInt();
				double pdop = linesplit[4].toDouble();
				double hdop = linesplit[5].toDouble();
				double vdop = linesplit[6].toDouble();
				emit signalStatus(fixtype,pdop,hdop,vdop);
			}
		}
	}
	if (m_buffer.contains("\n"))
